...
function ActivityRow({ item, isLast }: { item: ActivityItem; isLast: boolean }) {
  ...
  const { Icon } = cfg;
  return (
    <div className="flex items-start gap-3 py-3 px-2 rounded-xl hover:bg-white/[0.02] transition-colors cursor-default group">
      {/* Icon + connector line */}
      <div className="flex flex-col items-center gap-1 mt-0.5 flex-shrink-0">
        ...
        {!isLast && <div className="w-px flex-1 min-h-[16px] bg-white/[0.04]" />}
      </div>
      {/* Content */}
  ...
  return (
    <GlassCard>
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="text-[14px] font-semibold text-[#e4e9f5]">Recent Activity</h3>
